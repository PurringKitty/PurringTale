...
{
	public class LastAeigisBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / LastAeigisHoldout.NumBeams;
		...
		public override void AI() {
			...
			if (Projectile.type != ModContent.ProjectileType<LastAeigisBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<LastAeigisHoldout>()) {
				Projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / LastAeigisHoldout.MaxCharge, 0f, 1f);
			...
			Projectile.friendly = hostPrism.ai[0] > LastAeigisHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - LastAeigisHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * LastAeigisHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor() {
			...
			float hue = BeamID / LastAeigisHoldout.NumBeams % BeamHueVariance + BeamColorHue;
